    ...
    def get_logits_fn_(y):
        if not shallow_fusion:
            fscores = get_fscore_logits_from_decoders(
                dec_tm, dec_lm, dec_clm, y, T1, T2, a, L1, topk=K * 4)
            if normalize:
...
class Inference:
    ...
    def translate_docs_batch(self, docs, n_ctx, beam_size, maxlen_ratio=1.5, normalize=False, T1=1.0, T2=1.0, lp=0.0, a=0, L1=1.0):
        src_v, trg_v = self.vocab_src, self.vocab_trg
...
def main(argv, in_fp):
    ...
    p.add_argument('--mode', choices=['translate', 'fscore', 'rerank', 'instance'],
        default='translate')
